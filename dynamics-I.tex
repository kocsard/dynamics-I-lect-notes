...
\begin{theorem}[Topological conjugacy of expanding circle maps]
  If $f_1,f_2\in C^1_d(\T)$ are expanding maps, then there is a homeomorphism $h\in\Homeo+(\T)$ such that $h\circ f_1=f_2\circ h$.
\end{theorem}

\begin{proof}
  Let $\tilde f_1,\tilde f_2\colon\R\carr$ be two lifts of $f_1$ and $f_2$, respectively, with $\tilde f_i=id=\Delta_u$ and $\Delta_i\in C^0(\T,\R)$.  We shall prove that there are unique $u\in C^0(\T,\R)$ such that if we write $\tilde h=id+u$, it holds $\tilde h\circ\tilde f_1=\tilde f_2\circ\tilde h$. To do this, we first notice that since $f_1$ and $f_2$ are expanding, there exist $C>0$ and $\lambda>1$ such that $\abs{D\tilde f^n_i(x)}\geq C\lambda^n$, for every $x\in\R$, $n\in\N$ and $i\in\{1,2\}$. Let $n_0\in\N$ such that $C\lambda^{n_0}<1$. Then, notice that the equation $\tilde h\circ\tilde f_1^{n_0}=\tilde f_2^{n_0}\circ\tilde h$ induces the following functional equation for $u$:
