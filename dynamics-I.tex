...
\begin{proof}[Proof of Theorem~\ref{thm:Morse-Smale-circle-diffeomorphisms}]
  ...
  In order to see that $MS^r$ is $C^r$-dense in $\Diff{+}r(\T)$, let us assume $r$ is finite and fix an arbitrary $f\in\Diff{+}r(\T)$ and let $\tilde f\in\Thomeo_+(\T)$ be any lift of $f$. If $\rho(\tilde f)\in\Q$, then by Corollary~\ref{cor:rational-rotation-number-periodic-point} there is a periodic point for $f$. If $\rho(\tilde f)\in\R\setminus\Q$, then by Theorem~\ref{thm:closing-lemma-circle}, for every $\eps>0$ there is $g\in\Diff{+}r(\T)$ with $d_{C^r}(f,g)<\eps$ and such that $\rho(\tilde g)\in\Q$, where $\tilde g$ is any lift of $g$.
