...
\begin{theorem}[Spectrum and spectral radius]\label{thm:spectrum-spectral-radius}
  ...
  On the other hand, if $A\in\GL(E)$, then it holds
...
\begin{definition}[Hyperbolic linear automorphisms]
  Given a Banach space $E$, n invertible linear automorphism $A\in\GL(E)$ is said to be \textbf{hyperbolic} when it holds
  \begin{displaymath}
  ...
  \end{equation}
  Any number $\lambda\in(0,1)$ satisfying condition~\eqref{eq:hyp-constant-def} is called a \textbf{hyperbolicity constant} for $A$.
\end{definition}
...
\begin{theorem}[Hyperbolic linear maps]\label{thm:hyperbolic-linear-maps}
  ...
  Moreover, for every $\mu\in \big(A_{h},1\big)$, there exists a norm $\abs{\cdot}$ on $E$, which is equivalent to $\norm{\cdot}$, and such that
