...
\begin{proof}[Proof of Theorem~\ref{thm:Ed-topological-factor}]
  Let $f\in C_d^0(\T)$ be arbitrary and $\tilde f\colon\R\carr$ be any lift of $f$. We know that $\tilde f(x)=dx+\Delta(x)$, where $\Delta\in C^0(\T,\R)$. Looking for a map $h\in C^0_1(\T)$ such that $h\circ f=E_d\circ h$, lifting all the three maps to the universal cover, we get that $\tilde h\circ\tilde f(\tilde x)=d\tilde h(\tilde x)$, for every $\tilde x\in\R$. Since $h\in C^0_1(\T)$, its lift $\tilde h$ can be written as $\tilde h(\tilde x)=\tilde x+u(x)$, for every $\tilde x\in\R$, where $u\in C^0(\T,\R)$ is the unknown function that we need to find. Replacing this expression in the previous equation, we get that
  \begin{displaymath}
