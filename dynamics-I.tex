...
If $a\neq 1$ and $b\in\R$ is arbitrary, then $p_{a,b}=-\frac{b}{a-1}$ is the unique fixed point of $f_{a,b}$. Moreover, if $|a|<1$, then $f_{a,b}^n(x)\to p_{a,b}$, as $n\to+\infty$, for every $x\in\R$. If $|a|>1$, then $f_{a,b}^n(x)\to+\infty$ or $f_{a,b}^n(x)\to-\infty$, as $n\to+\infty$, for every $x\in\R\setminus\{p_{a,b}\}$.
