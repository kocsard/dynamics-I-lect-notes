...
\begin{proof}
  ...
  Let us choose points $\tilde x_i\in\pi^{-1}(x_i)\cap [0,1]$, for $i\in\{1,2\}$. Since $h(x_1)=h(x_2)$, we have $m:=\tilde h(\tilde x_2)-\tilde h(x_1)\in\Z$. Let us show that $m\neq 0$. If $m=0$, then $\tilde h(\tilde x_1)=\tilde h(\tilde x_2)$ and hence, we have
  \begin{displaymath}
    \tilde h\big(\tilde f^n(\tilde x_1)\big) = \tilde E_d^n\big(\tilde h(\tilde x_1)\big) = \tilde E_d^n\big(\tilde h(\tilde x_2)\big) = \tilde h\big(\tilde f^n(\tilde x_2)\big), \quad\forall n\in\N.
  \end{displaymath}

  Since $f$ is an expanding map, there is $C>0$ and $\lambda>1$ such that $\abs{Df^n(x)}=\abs{D\tilde f^n(x)}\geq C\lambda^n$, for every $x\in\T$ and $n\in\N$.
\end{proof}
